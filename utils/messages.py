...
def admin_help() -> str:
    return (
        ...
        f"<code>/dumps</code> / <code>/dumpinfo ID</code> — security dumps (owner + sudo)\n"
        f"<code>/clear USER</code> — backup + reset economy, returns recovery ID (owner)\n"
        f"<code>/recover ID</code> — restore economy from a dump (owner)\n"
        f"<code>/restore</code> / <code>/restorecase</code> — restore operations (owner)\n"
        f"<code>/securityset field value</code> — security config (owner)\n"
